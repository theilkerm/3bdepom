...
if(isset($_POST['name'])){
    //prepare the order
    $TASARIM = $DB->prepare("INSERT INTO `designs` (`name`, `description`, `category`, `creation_time`, `user_id`)
    VALUES ('name', 'description', 'category_id', now(), 'user_id');");

    $TASARIM->bindParam(":name", $_POST["name"]);
    $TASARIM->bindParam(":desctription", $_POST["desctription"]);
    $TASARIM->bindParam(":category_id",   $_POST["category_id"]);
    $TASARIM->bindParam(":user_id", $_POST["user_id"]);
}
